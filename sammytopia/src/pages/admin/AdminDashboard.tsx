...
const CONTENT_TYPES = [
  ...
  "gallery_item", "video", "baking_post", "cooking_post", "honours_award",
];

const MEDIA_CATEGORIES = [
  "wilberforce", "sammytopia", "school", "zamar", "baking", "about", "creative-work", "uncategorized",
];
...
export default function AdminDashboard() {
  ...
  const refreshContent = () => api.admin.list().then(setItems).catch(() => navigate("/admin"));
  const refreshMedia = (category = mediaFilter) =>
    api.admin.listMedia(category === "all" ? undefined : category).then(setMedia);
  ...
  const deleteMediaItem = async (id: string) => {
    ...
    await api.admin.removeMedia(id);
    refreshMedia();
  };
    ...
    <div className="section" style={{ background: "var(--parchment)", color: "var(--ink)", minHeight: "100vh" }}>
      <div className="container">
        ...
        <form onSubmit={save} className="admin-form" style={{ margin: 0, maxWidth: 640 }}>
          ...
          <label>Parent slug or ID (optional)</label>
        ...
        <h2 style={{ marginTop: 40 }}>Media library</h2>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          {["all", ...MEDIA_CATEGORIES].map((c) => (
            <button
              key={c}
              onClick={() => { setMediaFilter(c); refreshMedia(c); }}
              style={{
                background: mediaFilter === c ? "var(--gold)" : "transparent",
                border: "1px solid var(--paper-line)", borderRadius: 3, padding: "4px 10px", cursor: "pointer",
              }}
            >
              {c}
            </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
          {media.map((m) => (
            <div key={m.id} style={{ border: "1px solid var(--paper-line)", padding: 8 }}>
              {m.kind === "image" ? (
                <img src={`/media/${m.r2_key}`} alt={m.title ?? ""} style={{ width: "100%", aspectRatio: "1", objectFit: "cover" }} />
              ) : (
                <video src={`/media/${m.r2_key}`} style={{ width: "100%" }} muted />
              )}
              <p style={{ fontSize: "0.75rem", margin: "4px 0" }}>{m.title}</p>
              <button onClick={() => deleteMediaItem(m.id)} style={{ fontSize: "0.75rem", color: "var(--ember)", cursor: "pointer" }}>
                Delete
              </button>
            </div>
          ))}
